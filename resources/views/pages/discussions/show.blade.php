        ...
        <div class="container">
            ...
            <div class="row">
                <div class="col-12 col-lg-8 mb-5 mb-lg-0">
                                ...
                                <div class="d-flex justify-content-between align-items-center text-start">
                                    <div class="me-1 me-lg-2 d-flex flex-column gap-2">
                                        <div>
                                            ...
                                            @if ($discussion->status === 'updated')
                                                <span class="badge rounded-pill text-bg-light ps-2 fw-normal fst-italic">Edited {{ $discussion->updated_at->diffForHumans() }}</span>
                                            @endif
                                        </div>                                     
                                           ...
                                           <input type="text" id="current-url" value="{{ route('discussion.show', $discussion->slug) }}" class="d-none">
                                            ...
                                            @auth
                                                ...
                                                @if($discussion->user_id === Auth::user()->id)
                                                    ...
                                                    <!-- Tombol Delete -->
                                                    <form method="POST" action="{{ route('discussion.destroy', $discussion) }}" onsubmit="return confirm('Apakah Anda yakin ingin menghapus ini?')">
                                                        @csrf
                                                        @method('DELETE')
                                    ...
                                    <div class="d-flex align-items-center">
                                        ...
                                        <span class="fs-12px d-flex flex-column">
                                            <a href="{{ route('profile.index', $discussion->user->username) }}" class="me-1 fw-bold">{{ $discussion->user->name }}</a>
                    ...
                    <h3 class="mb-4">{{ $answers->count().' '. Str::plural('Answer', $answers->count()) }}</h3>
                                ...
                                <div class="col-12 col-lg-10">                            
                                    ...
                                    <div class="d-flex justify-content-between align-items-center text-start">
                                        <div class="me-1 me-lg-2 d-flex flex-column gap-2">
                                            <div class="d-flex flex-row gap-1 align-items-center p-1">
                                                @auth
                                                    {{-- muncul ketika user sesuai dengan yg membuat answer --}}
                                                    @if($answer->user_id === Auth::user()->id)
                                                        <!-- Tombol Edit -->
                                                        <a href="{{ route('answer.edit', Crypt::encryptString($answer->id)) }}" class="btn btn-light rounded-circle d-flex justify-content-center align-items-center p-0" style="width:25px; height:25px;" title="Edit" >
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil" viewBox="0 0 16 16">
                                                                <path d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325"/>
                                                            </svg>
                                                        </a>
                                                        ...
                                                        <!-- Tombol Delete -->
                                                        <form method="POST" action="{{ route('answer.destroy', $answer) }}" onsubmit="return confirm('Apakah Anda yakin ingin menghapus ini?')">
                                                            @csrf
                                                            @method('DELETE')
                                        ...
                                        <div class="d-flex align-items-center">
                                            ...
                                            <span class="fs-12px d-flex flex-column">
                                                <a href="{{ route('profile.index', $answer->user->username) }}" class="me-1 fw-bold">{{ $answer->user->name }} 
                    ...
                    @empty
                        <div class="card card-discussion mb-4">
                            Currently, there is no answer yet 
                        </div>
                    @endforelse
